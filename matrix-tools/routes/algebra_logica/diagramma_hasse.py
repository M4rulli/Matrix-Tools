...
def compute_hasse_edges(S: list[int]) -> list[tuple[int,int]]:
    ...
    for i, a in enumerate(S):
        for b in S[i+1:]:
            if b % a != 0:
                continue
            # controlla se esiste un c intermedio
...
def verifica_reticolo(S: list[int]) -> tuple[bool, list[tuple[int, int, str]]]:
    ...
    for i, a in enumerate(S):
        for b in S[i:]:
            # Calcola sup (minimo tra i multipli comuni in S)
            multipli = [z for z in S if z % a == 0 and z % b == 0]
            sup = min(multipli) if multipli else None

            # Calcola inf (massimo tra i divisori comuni in S)
...
def genera_hasse_tikz(nodes: list[int], edges: list[tuple[int,int]]) -> str:
    ...
    """
    # calcola profondità di ciascun nodo (numero di coperture minori)
    depth = {nodes[0]: 0}
    for node in nodes[1:]:
...
def diagramma_hasse():
    ...
    try:
        ...
        if data["type"] == "n":
            # Notazione D_n per l'insieme dei divisori di n
        ...
        # Calcola atomi (elementi coperti dal minimo se esiste un minimo)
        atoms = []
        if minimals:
            min_el = min(minimals)
            # Atomi: elementi che coprono il minimo
            atoms = [b for (a, b) in edges if a == min_el]
        # Calcola ∨-irriducibili (join-irreducibili) per divisibilità (lcm)
        from math import gcd
        from functools import reduce
...
"""
        ...
        if is_lattice:
            ...
            # Step 2: elenco coppie problematiche in MathJax con virgole e spazi
            notin = '\\notin'
            text_cmd = '\\text'
            coppie_str = ', '.join(
