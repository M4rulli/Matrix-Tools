...
def identita_bezout_route():
    ...
    # Step 1: iterazioni algoritmo di Euclide esteso con tracciamento dei coefficienti di Bézout
    iterations = []
    r0, r1 = a, b
    ...
    # costruisci mappa di sostituzione per r_i -> rhs
    mapping = []
    ...
    # inizia con l'ultima equazione (gcd)
    last_line = back_steps[-1]
    lhs, rhs = [p.strip() for p in last_line.split("&=")]
    # costruisci l'elenco di sostituzioni progressive, con wrap che ripete sotto '='
    expr = rhs
    subs_lines = []
    # prima sostituzione: linea base
    parts = split_long(expr)
    if parts:
        subs_lines.append(f"{lhs} &= {parts[0]}")
        for cont in parts[1:]:
            subs_lines.append(f"& {cont}")
    # applica le restanti sostituzioni a ritroso
