...
# ---------- Algoritmo di ricerca riga/colonna con più zeri ---------------- #
def best_row_or_col(mat: List[List[float]]) -> Tuple[str, int, Set[Tuple[int, int]]]:
    ...
    if max_row_zeros >= max_col_zeros:  # preferisci riga
...
# ---------- Determinante (ricorsivo) con passi ---------------------------- #
def determinant(mat: List[List[float]],
    ...
    if n == 2:
        ...
        c, d = mat[1]
        det_val = a * d - b * c
        # Step LaTeX per 2×2 (solo se non siamo nel livello più profondo)
    ...
    # --------------- Selezione riga/colonna ottimale ---------------
    ...
    def minor_matrix(m: List[List[float]], r: int, c: int) -> List[List[float]]:
        """Sottomatrice eliminando riga r e colonna c."""
        return [ [m[i][j] for j in n_range if j != c]
                 for i in n_range if i != r ]

    # Prepara la lista di posizioni per riga o colonna scelta
    ...
    # Ora ciclo reale per calcolo numerico
...
def determinante_laplace_route():
    ...
    # Conversione numerica con supporto a frazioni LaTeX (\frac)
    ...
    # Passo 0 – Matrice iniziale
    steps.append({
        "title": "Matrice iniziale:",
        "content": latex_matrix(mat)
    })

    # Calcolo determinante con passi
