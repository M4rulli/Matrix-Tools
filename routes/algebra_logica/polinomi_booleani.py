...
# Funzione per output LaTeX con apice per la negazione
...
def parse_boolean_expr(expr_str):
    ...
    # --- nuova sezione: gestisci i doppi NOT -------------------------------
    ...
    # 3. Notazione a token: ^doubleprime  oppure ''  →  vuoto
    ...
    while "'" in expr_str:
        ...
        for j in range(p - 2, -1, -1):
            ...
            if c == ')':
                ...
                depth -= 1
        if open_idx is None:
            # parentesi non bilanciate, rimuovi l'apice e continua
            expr_str = expr_str[:p] + expr_str[p + 1:]
            continue

        # sostituisci il blocco con la forma negata
...
def _consensus(lits1, lits2):
    ...
    """
    # --- individua le variabili con polarità opposta --------------------
    diff_var = None
    for var, pol in lits1:
        if (var, pol) not in lits2 and (var, not pol) in lits2:
            # trovato var con polarità opposta
...
def to_fnd(expr_str, var_names=None):
    ...
    # 1) Determina le variabili che l'utente desidera visualizzare
    if var_names is None:
        vars_sym = sorted(expr.free_symbols, key=lambda s: s.name)
        var_names = [str(v) for v in vars_sym]
    else:
        # crea i simboli nell'ordine richiesto (anche se non compaiono in expr)
    ...
    # 3) Costruisci truth table e lista mintermini via eval()
    tt_rows = []
    minterms = []
    ...
    # 5) Costruisci la FND canonica esatta (somma dei mintermini, non ridotta)
    if not minterms:
