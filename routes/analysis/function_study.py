...
def safe_domain(expr, x):
    ...
    try:
        ...
        # Il domain non può essere calcolato in modo elementare (es. disuguaglianza trascendentale):
        # ripieghiamo su ℝ senza bloccare il flusso.
        base_domain = S.Reals
    except Exception:
        # Qualsiasi altra eccezione imprevista: continuous with domain reale completo
        base_domain = S.Reals
    ...
    for p in expr.atoms(Pow):
        ...
        if isinstance(zeros, sp.FiniteSet):
            # If l’esponente è negativo o simbolico → escludi i punti dove la base è 0
            if (expo.is_number and expo.evalf() < 0) or (expo.has(x)):
                forbidden_points = forbidden_points.union(zeros)

        # 2. If l’esponente è razionale with denominatore dispari → accettiamo basi anche negative
        if expo.is_Rational and expo.q % 2 == 1:
            continue  # full domain ℝ

        # 3. If l’esponente non è garantito intero → imponi base ≥ 0
...
def evaluate_inf_sign(expr):
    ...
    def _real_sign(val):
        # Caso Pow with base reale negativa e esponente razionale p/q (q dispari):
        # (-a)**(p/q) ha segno (-1)**p
        ...
        # Fallback: use the numeric real part of val
        try:
...
# ----------------------------------------------------------------------
# API endpoint per lo studio della function
# ----------------------------------------------------------------------
@studio_funzione_bp.route("/api/function-study", methods=["POST"])
...
def studio_funzione():
    ...
    expr = expr.replace("^", "**")
    # Remove eventuali empty exponent braces e asterischi doppi vuoti (es. e^{}^{})
    expr = expr.replace("**{}", "")
    ...
    # Potenze with graffe: x**{cos x} → x**(cos x)
    ...
    try:
        # ── Minima patch: rimuove Abs() per potenze razionali with denominatore dispari
    ...
    # ------------------------------------------------------------
    # Riscrittura della function if contiene Abs
    ...
    # ------------------------------------------------------------
    # 2. Domain (ristretto if periodica)
    # ------------------------------------------------------------
    try:
    ...
    def rewrite_abs_piecewise(expr, x, dom_f):
        ...
        for abs_expr in abs_vars:
            ...
            # Evita che SymPy normalizzi l'ultimo ramo with True
    ...
    if f_expr.has(Abs):
        ...
        for branch_expr, branch_cond in flattened:
            ...
            # Compute la condizione di ramo suddividendo with solveset e intersection
            ...
            cond_set = dom  # start from the main domain
            for cond_i in conds:
                try:
    ...
    # ------------------------------------------------------------
    # 4. Intersection with l'asse y (f(0))
    # ------------------------------------------------------------
    try:
    ...
    # ------------------------------------------------------------
    # 5. Intersection with l'asse x (zeri)
    try:
        ...
        # No real intersection: still include the step
    ...
    try:
        # Solve direttamente l'inequazione f_expr > 0 sul domain
        sol = solveset(f_expr > 0, x, domain=dom)
        sign_latex = sp.latex(sol)
        # Show '+' before infinity
        ...
        # If l'insieme positivo è vuoto, aggiungi la row extra
    ...
    # Prepara i punti di singolarità da riutilizzare nello step degli asymptotes verticali
    ...
    # Estremi di accumulazione ai bordi del domain
    ...
    for interv in intervals:
        if isinstance(interv, sp.Interval):
            ...
            # If a è un estremo finito ed è aperto ⇒ limite da destra (x → a⁺)
            if a.is_finite and left_open:
                try:
                    lim_right = sp.limit(f_expr, x, a, dir="+")
                    # Normalizza with evaluate_inf_sign
            ...
            # If b è un estremo finito ed è aperto ⇒ limite da sinistra (x → b⁻)
    ...
    asintoti = []
    # Limiti laterali in punti di singolarità per asymptotes verticali
    ...
    # Asymptotes orizzontali: limiti per x→±∞
    horizontal_vals = []
    for inf, label in [(sp.oo, r"+\infty"), (-sp.oo, r"-\infty")]:
        try:
            ...
            lim_inf = None
        # Only if limite esiste ed è finito
    ...
    # 9. Asintoto obliquo (dopo Asymptotes)
    oblique_lines = []
    ...
    # Classificazione secondo le 3 specie with giustificazioni
    for a, lims in point_limits.items():
        ...
        if l is None or r is None:
            ...
            # Limiti uguali (l == r)
            # If sono ±∞ ⇒ discontinuità di seconda specie (limite infinito bilatero)
            ...
            # Altrimenti sono finiti e coincidenti → controlla valore della function
            ...
            if not in_domain:
                # Punto fuori dal domain ma limiti coincidenti ⇒ eliminabile
                discontinuities.append(
                    rf"x={a}: \lim_{{x\to {a}^\pm}}f(x)={l},\ "
                ...
                try:
                    ...
                    # If fa_tex == l ⇒ punto di continuità, no discontinuità
                except Exception:
                    discontinuities.append(
    ...
    # ------------------------------------------------------------
    # 11. Derivative della function a tratti
    # ------------------------------------------------------------
    # Use the piecewise rewrite computed above (abs_piecewise_expr)
    from sympy import Eq, Piecewise
    # Determina la function da derivare: preferisci abs_piecewise_expr
    f_pw = abs_piecewise_expr if abs_piecewise_expr is not None else f_expr
    # Compute la derivative di f_pw
    f_prime_raw = sp.diff(f_pw, x).simplify()
    # Avvolgi in Piecewise if necessario
    if isinstance(f_prime_raw, Piecewise):
        f_prime_pw = f_prime_raw
    else:
        f_prime_pw = Piecewise((f_prime_raw, True))
    # Remove eventuali branch centrali non derivabili (Eq)
    cleaned = []
    ...
    # If non è Piecewise, f_prime_pw resta com'è
    # Prepara LaTeX per output
    # If the derivative is Piecewise with a single branch, show only the expression (without braces)
    ...
    # ------------------------------------------------------------
    try:
        # Compute il domain di f e di f'
        f_prime_sym = f_prime_pw
        try:
            # Compute la continuità di f' only all’interno del domain ristretto
            cont_dom_fprime = continuous_domain(f_prime_sym, x, dom)
        except Exception:
            cont_dom_fprime = dom
        # raw ND = domain ristretto \ Dom(f')
        ...
        # Piecewise: if il punto appartiene al domain di f, deve essere
        # considerato candidato punto non derivabile (es. punto angoloso).
        # If invece il punto NON appartiene al domain di f, viene ignorato.
        # ------------------------------------------------------------------
        junction_pts = set()
        ...
        # Mantieni only i punti che appartengono al domain principale di f
...
= {nd_tex}"""
    ...
    # Filtro sui punti per includere only quelli nel domain di f
    # Compute il domain di continuità della function originale
    ...
    try:
        ...
        cont_dom_f = S.Reals
    # Seleziona only i punti non derivabili che appartengono al domain di f
    valid_pts = [p for p in punti_non_derivabilita if cont_dom_f.contains(p) is S.true]
    # If there are no points to analyze, show the message and return
    if not valid_pts:
        ...
        def latex_limit(val):
            # 1) If contiene parte immaginaria non nulla
            try:
        ...
        for punto in valid_pts:
            # Computation limiti unilaterali tramite rapporto incrementale
            h = sp.Symbol('h', real=True)
            x0_str = sp.latex(punto)
            # Rapporto incrementale base with x0
        ...
        # Unico blocco align* with tutti i punti uno sotto l'altro
        steps[-1]["content"] = r"\begin{align*}" + r"\\ ".join(lines) + r"\end{align*}"

    # ------------------------------------------------------------
    # 14. Monotonia – intervalli di crescita/decrescita (f′ ≥ 0 / f′ ≤ 0)
    # -------------------------------------------------------------------
    # Initialize inc/dec: servono poi allo step 16
    inc_set = sp.S.EmptySet
    dec_set = sp.S.EmptySet

    try:
        # Use the derivative already computed (it may be Piecewise)
        ...
        for branch_expr, branch_cond in branches:
            ...
            # --- replace x → x_real per coerenza with le variables usate nei solver
            branch_simpl = branch_simpl.xreplace({x: x_real})

            # 3) Domain del ramo
            try:
                # use safe_domain to correctly handle rational powers
            ...
            # 4) Intervalli inc / dec sul domain del ramo usando sign-chart
    ...
    try:
        ...
        # dec_set è già state calcolato allo step 15 (Monotonia) e contiene il
        # domain di decrescenza; non lo ricalcoliamo qui to avoid loss di info.
        ...
        for (iv1_int, t1), (iv2_int, t2) in zip(intervals_all, intervals_all[1:]):
            ...
            if boundary is not None and boundary.is_real:
                # Salta if il punto non appartiene al domain della function
        ...
        # --- Computation del valore di f in ciascun punto di estremo relativo ---
        extrema_lines = []

        # --- raccogli info ---
        info = []   # list of dicts: {"x": p, "val": f_val, "nature": "max"/"min"}
        for p, nature in extrema_pts:
            # ignora eventuali punti non nel domain (sicurezza aggiuntiva)
    ...
    def to_js_func(expr: sp.Expr) -> str:
        ...
        js_str = str(expr)
        # Replace pi with il valore numerico per renderlo riconoscibile in JS
    ...
    plot_data = {
        # stringa della function per il plotter JS
        "func": to_js_func(f_expr),
        # intervalli continui del domain: [[a,b], ...] (None = ±∞)
        "domain": [],
    ...
    # Build gli intervalli continui del domain per il front‑end
    ...
    # Build la lista di estremi with valori reali
    extrema_list = []
    for p, nature in extrema_pts:
        ...
        if getattr(p, "is_real", False):
            try:
                ...
                y_num = sp.N(y_val)
                # Includi only if è un reale finito
                if y_num.is_real:
                    extrema_list.append({
                        "x": float(p),
                        "y": float(y_num),
