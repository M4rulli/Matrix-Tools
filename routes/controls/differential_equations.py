...
@equazioni_differenziali_bp.route('/api/differential-equations', methods=['POST'])
...
def normalize_derivatives(equation_str):
    ...
    # If la forma è semplicemente "d^n" o "d**n", converti in forma completa
...
def solve_differential_equation(equation_str, condizioni_iniziali=None):
    try:
        ...
        # If sono fornite condizioni iniziali, devono essere pari all’order
        ...
        # Caso speciale: termine forzante zero → only solution homogeneous
        if rhs_expr == 0:
            # Risolvo only l’homogeneous
            homogeneous_sol = dsolve(Eq(lhs_expr, 0), y)
            # Estraggo direttamente la solution (non lista)
            homogeneous_rhs = collect(simplify(homogeneous_sol.rhs), t)
            homogeneous_latex = f"y_o(t) = {latex(homogeneous_rhs)}"

            # Aggiungo lo step per l’homogeneous pura
            latex_steps.append({
        ...
        # Computation dell'annichilatore A(Δ)
        try:
        ...
        # ------------------------------------------------------------------
        #  Estrai dal lato sinistro ONLY il polinomio P(Δ), rimuovendo
        ...
        # --------------------------------------------------
        # BUILD ONLY IL POLINOMIO OPERATORE (NO y, NO Derivative)
        # --------------------------------------------------
        full_operator = sp.expand(A_expr * P_operator)
        # sicurezza: rimuove eventuali Derivative(y, t, ...) rimasti per error
        ...
        try:
            ...
            # Rimpiazza le costanti C_i with α_i per coerenza visiva
        ...
        # Solution homogeneous (prova sempre, fallback su error)
        try:
            ...
            homogeneous_rhs = 0  # fallback to avoid errors

        # Computation della solution particular automaticamente via dsolve
        try:
            particular_sol = dsolve(original_eq, y)
            # estrai l'RHS e sottrai la solution homogeneous per ottenere la particular
            ...
            # If la solution particular è in fase, la espande in forme sin+cos
        ...
        # Compute la solution generale y(t) = y_o(t) + y_p(t)
        try:
            split = split_phase(particular_rhs, t)
        ...
        # Genera il LaTeX with i simboli già corretti
        ...
        if condizioni_iniziali:
            ...
            if sol_consts:
                ...
                # --------------------------------------------------
                # Apply symbolic solutions to the general form
                # --------------------------------------------------
                # --------------------------------------------------
