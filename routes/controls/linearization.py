def latex_to_sympy_str(expr):
    ...
    # Substitution pedici: x_1 -> x1
    expr = re.sub(r'x_(\d+)', r'x\1', expr)
    expr = re.sub(r'x_\{(\d+)\}', r'x\1', expr)
    # Handle powers: x^3 -> x**3, x^{3} -> x**3
    expr = re.sub(r'\^(\{[^{}]+\}|[a-zA-Z0-9\(])', lambda m: '**' + m.group(1).strip('{}'), expr)
    # Moltiplicazione implicita: inserisce * tra numeri, variables e parentesi adiacenti
...
@linearizzazione_bp.route('/api/linearization', methods=['POST'])
...
def linearizzazione():
    try:
        ...
        # Remove \left e \right anche da valore_ingresso_str
        ...
        # Definisci le variables simboliche x1, x2, ..., xn
        x = symbols(f'x1:{numero_equazioni+1}')  # Genera x1, x2, ..., xn
        from sympy.abc import u

        # Pre-elabora le equations
        eqs_sostituite = []
        f_exprs = []
        for eq in equazioni:
            try:
        ...
        # Build the equations for the equilibrium point
        ...
        # DEBUG: stampa il system di equations in forma Python e LaTeX
        # Remove only tautological equations 0=0, while preserving Eq(x1,0)
        eqs_da_risolvere = []
        ...
        # --- FILTRO: keep only real solutions ---
        def is_real_expr(expr):
            ...
            if expr is None:
                return False
            # use the is_real property when available
        ...
        # If dopo tutto non ci sono solutions, creane una sottodeterminata generica
        if not soluzioni:
        ...
        # Replace simboli with espressioni semplici if sono riferimenti ad altri simboli o costanti
        ...
        for idx, sol in enumerate(soluzioni_reali):
            # semplifica espressioni tra variables nel punto di lavoro
            changed = True
            while changed:
                changed = False
                for k, v in sol.items():
            ...
            # build substitution dictionary per x_i → c_i
        ...
        # Verifica if ci sono solutions reali
        ...
        for idx, sol in enumerate(soluzioni_reali):
            ...
            # Recreate subs_dict per sostituire x_i → c_i
            subs_dict = { x[j]: sp.Symbol(f"c_{j+1}") for j in range(numero_equazioni) }
            ...
            try:
                ...
                # If the point has c_i parameters, replace x_i->c_i without evalf; otherwise use evalf
                ...
                # Coerente with il parsing delle equations
                ...
                # Use the same subs_dict above to replace x_i -> c_i
