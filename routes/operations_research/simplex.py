...
------------------------
{
  ...
  "n"         : 3,                      # number of constraints (currently 2–4)
  "obj"       : "3x_1 + 2x_2",          # objective function
...
def _parse_expr(expr: str) -> sp.Expr:
    ...
    # Insert the implicit multiplication symbol: e.g., 3x_1 -> 3*x_1
    s = re.sub(r'(?<=[0-9\)])(x_\d+)', r'*\1', s)

    # replace x_i → x_i (sympy symbols)
    tokens = _var_re.findall(s)
    syms   = {f"x_{i}": sp.symbols(f"x_{i}") for i in tokens}
    # NB: sympy non accetta _ nelle variables senza quoting: usiamo x_1 come sympy Symbol name
    return sp.sympify(s, locals=syms)

# ---------- ROUTE -------------------------------------------------------- #
@simplesso_bp.route("/api/simplex", methods=["POST"])
...
def simplesso_route():
    ...
    # colleziona tutte le variables x_i presenti
    max_index = 0
    ...
    # elenco esplicito variables non negative
    ...
    # elenco esplicito variables e slack non negative
    ...
    # variables di base = slack, non–base = originali
    dict_lines = []
    for i in range(n):
        bi  = rhs_vec[i]
        row = lhs_list[i]
    ...
    # function obiettivo (convertita a massimizzazione if necessario)
    z_expr   = obj_expr

    # esprimi z in function delle variables NON di base (replace le base)
    ...
    # ---------------------------------------------------------
    #  Fase I: verifica if il dizionario iniziale è ammissibile
    # ---------------------------------------------------------
    ...
    for it in range(1, MAX_ITERS+1):
        ...
        # replace in tutte le altre righe
        for b in base_vars:
            if b == leaving:
                continue
            dict_expr[b] = dict_expr[b].subs({entering: new_rhs})
        z_expr = z_expr.subs({entering: new_rhs})
        # update dizionario: leaving diventa nonbase, entering base
