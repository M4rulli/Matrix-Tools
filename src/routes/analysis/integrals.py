# ---------------------------------------------------------------------------
#  ROUTE: /api/integrale – Computation integrale indefinito with steps
# ---------------------------------------------------------------------------
...
# =========================================================================== #
# --------------------- MOTORE DI INTEGRAZIONE WITH STEPS ------------------ #
# =========================================================================== #
#  • Ogni strategia aggiunge step LaTeX al log.
#  • La function principale `integrate_with_steps(expr, x)` restituisce
...
def _integrate_linearity(expr, x, steps):
    ...
    for t in terms:
        ...
        steps.extend(sub_steps)   # show all term-level steps
        total += prim_t
    return total
...
def _integrate_rational(expr, x, steps):
    ...
    if deg_num >= deg_den:
        ...
        # Show the rewritten quotient integral + proper part
        _add_step(
            steps,
            "Nuovo integrale da calcolare:",
        ...
        # Integra quoziente e parte propria R/den with steps completi
    ...
    # If la decomposizione cambia l'espressione, mostrala
    if apart_expr != expr:
    ...
    # If no decomposizione, prova SymPy di default
...
@integrali_bp.route("/api/integral", methods=["POST"])
...
def calcola_integrale():
    ...
    # ---------------------------------------------------------
    # Integra with steps dettagliati
    # ---------------------------------------------------------
    try:
