# routes/controls/difference_equations.py
...
# ------------------------------------------------------------
#  Normalizzazione LaTeX-like -> SymPy-like (discrete)
# ------------------------------------------------------------
def normalize_latex_like_discrete(expr: str) -> str:
    ...
    # ------------------------------------------------------------
    #  1) Remove token LaTeX di sizing PRIMA di toccare i backslash
    #     (altrimenti "\\left" diventa "left" e il parser lo interpreta
    #     come prodotto implicito l*e*f*t ...)
    # ------------------------------------------------------------
    # gestisci sia stringhe with doppio backslash ("\\\\left") che with singolo ("\\left")
    expr = re.sub(r"\\\\left\b", "", expr)
    expr = re.sub(r"\\\\right\b", "", expr)
    expr = re.sub(r"\\left\b", "", expr)
    expr = re.sub(r"\\right\b", "", expr)

    # difensivo: if per qualche motivo i backslash sono già stati rimossi
    # (non dovrebbe accadere prima del parsing)
    expr = re.sub(r"\bleft\b", "", expr)
    expr = re.sub(r"\bright\b", "", expr)

    # ------------------------------------------------------------
    #  2) Impulso unitario discrete: \delta_0( ... ) -> KroneckerDelta(t, k)
    #     Supporta forme:
    #       \delta_0(t), \delta_0(t-2), \delta_0(t+2)
    #    \delta_{0}(\left(t\right)) e varianti with/ senza backslash
    # ------------------------------------------------------------
    # normalizza indice: \delta_{0} -> \delta_0 (gestisci anche doppio backslash)
    expr = expr.replace(r"\\delta_{0}", r"\\delta_0")
    expr = expr.replace(r"\delta_{0}", r"\delta_0")

    # Prima gestisci i casi with shift (t-k) / (t+k) with o senza backslash.
    ...
    # Unicode delta (if mai arrivasse): δ_0(t)
    ...
    # Elimina eventuali backslash residui ONLY alla fine (difensivo).
    # Qui dovrebbero rimanere only backslash "inermi"; if resta qualche comando non riconosciuto,
...
# ------------------------------------------------------------
#  Utilities: delta discrete with ipotesi t>=0 (causale)
# ------------------------------------------------------------
...
def simplify_delta_t_nonneg(expr: sp.Expr, t_sym: sp.Symbol) -> sp.Expr:
    ...
    def _rule_kd(kd: sp.KroneckerDelta):
        ...
        # Normalizza l'order: vogliamo preferibilmente KD(t, k) (stile corso)
...
def get_annichilatore_discrete(rhs_expr: sp.Expr, t_sym: sp.Symbol, d_sym: sp.Symbol):
    ...
    def ann_term(term: sp.Expr):
        ...
        #     (Δ^2 - 2 cos(w) Δ + 1)
        #   e versioni with p^t: p^t cos(w t), p^t sin(w t)
        # -------------------------------------------------
...
# ------------------------------------------------------------
#  Solver (forward shift): P(Δ) y(t) = u(t),  t>=0
# ------------------------------------------------------------

def solve_difference_equation_shift(equation_str: str, condizioni_iniziali=None):
    """Endpoint solver for difference equations with forward shift.

    This version is designed for Flask backend usage:
      - normalizes LaTeX-like input
      - parses LHS/RHS into SymPy
      - extracts P(d) (polynomial in the shift operator)
      - simplifies impulsive forcings under t>=0
      - computes a discrete annihilator (lookup) for impulses

    Note: rsolve/solve is intentionally not used here (it often fails with KroneckerDelta).
    The result returns LaTeX steps and useful symbolic components.
    """
    ...
    def build_homogeneous_solution(P_d_expr: sp.Expr, t_sym: sp.Symbol, d_sym: sp.Symbol) -> sp.Expr:
        """Build y_o(t) for P(d) y = 0 with d as forward shift.

        - For the factor d^m: generates impulses c_0*KD(t,0)+...+c_{m-1}*KD(t,m-1)
          (consistent with domain t>=0: d^m y=0 => y(t)=0 for t>=m, while first m samples are free).
        - For non-zero roots λ with multiplicity r: sum_{j=0}^{r-1} c * t^j * λ^t
        """
        P_poly = sp.Poly(sp.expand(P_d_expr), d_sym)
        if P_poly.is_zero:
            return sp.Integer(0)

        # Multiplicity of the d factor
        m0 = 0
        tmp = P_poly
        while tmp.degree() > 0:
        ...
        #   if lam = ρ e^{iω} => ρ^t cos(ω t), ρ^t sin(ω t)
        #  (only estetica; solution equivalente in R)
        # --------------------------------------------------
        ...
        while unused:
            ...
            # If non troviamo il coniugato (o molteplicità diversa), fallback alla forma complessa
    ...
    def latex_with_discrete_delta(expr: sp.Expr) -> str:
        ...
        # Replace placeholders with delta_0(t-k) (course notation)
        for kd_obj, sym in repl.items():
            ...
            if not m:
                continue
            k = m.group(1)
            # Course notation: KD(t,k) = \delta_0(t-k)
            kk = int(k)
            if kk == 0:
    ...
    def build_particular_impulse_solution(P_d_expr: sp.Expr, u_expr: sp.Expr, t_sym: sp.Symbol, d_sym: sp.Symbol) -> sp.Expr | None:
        """Build a finite-support causal particular solution when u(t) is impulsive.

        Robust method (course-style, t>=0):
        1) Factorize P(d) = d^m Q(d) with Q(0) != 0.
        2) Define z(t) = y(t+m), so Q(d) z(t) = u(t).
        3) Impose the tail condition z(t)=0 for t > T (finite support) and solve backward:
              q0 z(t) + q1 z(t+1) + ... + qn z(t+n) = u(t)
           therefore:
              z(t) = (u(t) - sum_{i>=1} qi z(t+i)) / q0
        4) Reconstruct y(t) = z(t-m), i.e., in terms of impulses KD(t, k+m).

        Note: infinitely many particular solutions exist; this choice reproduces the
        standard impulse-based particular solution used in class.
        """
        ...
        # m = order del fattore d (valutazione a 0): P(d)=d^m Q(d), Q(0)!=0
        m = 0
        tmp = P_poly
        while tmp.degree() > 0:
            rem = tmp.rem(sp.Poly(d_sym, d_sym))
        ...
        q0 = q[0]
        if q0 == 0:
            # Non dovrebbe accadere if abbiamo rimosso tutti i fattori d
            return None
        ...
        # resolution all'indietro per z(t) da T a 0
        ...
        # build y_p(t) = z(t-m) -> impulsi a indice (tt+m)
        terms = []
    ...
    rhs_expr = simplify_delta_t_nonneg(rhs_expr, t)  # normalize KD argument order as well
    print("[DEBUG][diff] RHS parsed (simplified t>=0):", rhs_expr)

    # Estrai P(d) dal LHS rimuovendo y (if presente come simbolo letterale)
    # In questa route discreta ci aspettiamo che il frontend invii già P(d)*y oppure only P(d).
    y_sym = sp.Symbol("y")
    P_d = lhs_expr
    P_d = P_d.replace(lambda e: e == y_sym, lambda e: 1)
    P_d = sp.expand(P_d)
    print("[DEBUG][diff] P(d) extracted:", P_d)
    # Order dell'equation alle differenze: massimo shift (grado in d)
    ordine = int(sp.Poly(P_d, d).degree()) if not sp.Poly(P_d, d).is_zero else 0

    # If l'utente ha scritto qualcosa tipo (d-1)(d+1)y, allora P_d conterrà ancora y (if non era simbolo)
    # Difensivo: if resta una AppliedUndef o Function, segnala.
    ...
    # --------------------------------------------------
    # Solution dell'equation homogeneous: P(Δ) y(t) = 0
    # --------------------------------------------------
    ...
    # Annichilatore discrete (lookup) per impulsi
    ...
    if A_d is None:
        ...
        # Always show factorized form over R: (d+1)^2*(d^2+4), etc.
        ...
        # --------------------------------------------------
        # Solution dell'homogeneous estesa: (A(Δ)P(Δ)) y(t) = 0
        # --------------------------------------------------
    ...
    def solve_particular_coeffs_by_sampling(y_ansatz: sp.Expr, alpha_syms, rhs_u: sp.Expr, P_poly: sp.Expr, t_sym: sp.Symbol, d_sym: sp.Symbol):
        ...
        # genera system lineare su alpha valutando t=0..K
        m = len(alpha_syms)
        K = max(8, m + 4)
        eqs = []
        for k in range(K):
    ...
    # --------------------------------------------------
    # Solution particular (impulsiva o extended-homogeneous)
    # --------------------------------------------------
    y_p = build_particular_impulse_solution(P_d, rhs_expr, t, d)

    # If non è impulsiva e abbiamo A(Δ), derive i modi nuovi e solve i coefficienti (nessun alpha in output)
    ...
    # --------------------------------------------------
    # Solution generale (senza CI): y_g(t) = y_o(t) + y_p(t)
    # --------------------------------------------------
    ...
    # Useful debug: apply P(d) to the candidate particular solution if provided by the client
    # (opzionale)  payload: {"candidate": "-3*KroneckerDelta(t,2)"}

    # --------------------------------------------------
    # APPLICAZIONE CONDIZIONI INIZIALI (if presenti)
    # --------------------------------------------------
    if condizioni_iniziali:
        ...
        # Costanti libere c_i presenti nella solution generale
        ...
        # Regola: if una CI è una stringa ma rappresenta un numero/espressione numerica (es. "1", "-1/2", "sqrt(2)"),
        # la trattiamo come NUMERICA. Only stringhe with simboli (es. "y_0") attivano la modalità canonica.
        ci_vals = []
        ci_syms = []
        any_symbolic = False
        ...
        def _parse_ci(ci_raw):
            if isinstance(ci_raw, str):
                try:
                    v = sp.sympify(ci_raw)
                    # if non contiene simboli liberi, è numerica
        ...
        for ci in condizioni_iniziali:
            ...
            if is_symb:
                any_symbolic = True
                # registra only i simboli 'canonici' delle CI
                for s in sorted(v.free_symbols, key=lambda s: s.name):
                    if s not in ci_syms:
                        ci_syms.append(s)
            ci_vals.append(v)

        # System: y_g(i) = CI_i per i=0..order-1
        eqs = []
        ...
        # Build la solution finale
        y_ci = sp.simplify(sp.expand(y_g.subs(sol_c)))

        # Step: system (no need to restate initial conditions; show only equations in c_i)
        # Riscrivi le equations sostituendo i valori numerici/simbolici a destra
        latex_steps.append({
        ...
        if any_symbolic:
            ...
            #  - risposta forzata = tutti gli altri termini (dipendono only da t e dai parametri del system)
            # Nota: NON coincide in generale with y_p.
            # --------------------------------------------------
...
#  Route Flask
# ------------------------------------------------------------
@equazioni_differenze_bp.route("/api/difference-equations", methods=["POST"])
