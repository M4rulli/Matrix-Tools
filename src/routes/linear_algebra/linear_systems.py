...
JSON in uscita:
{
  ...
  "result": {
      ...
      "rankA": int,
      "rankAug": int,
      "solution": str | null          # solution LaTeX if unique / parametric
  }
}
"""
...
def latex_augmented(A: sp.Matrix, b: sp.Matrix, p: int) -> str:
    """Return LaTeX for augmented matrix [A | b] using square brackets."""
    n = A.rows
    cols_tex = []
    for i in range(n):
...
def latex_cases_system(eqs: List[List[str]]) -> str:
    """
    Return the system in the following format:
      { aligned
        lhs_1 &= rhs_1 \\
        lhs_2 &= rhs_2 \\ …
    with alignment on the equals sign.
    """
...
def _pivot_zeros(A: sp.Matrix, k_sym):
    """
    Return the set of k values that zero out pivots
    of matrix A (already in echelon form).
    """
    # --- pivot e denominatori calcolati direttamente su 'A' --------------
    crit = set()

    # 1) individua i pivot della forma a gradini: primo elemento non‑nullo di ogni row
...
# ---------- helper: fattorizza in k if possibile (only su ℝ) -------------
...
# ---------- route -------------------------------------------------------- #
@syslin_bp.route("/api/linear-systems", methods=["POST"])
...
def sistemi_lineari_route():
    ...
    # elenco variables ammesse in order
    ...
    for lhs_rhs in equations:
        ...
        if len(lhs_rhs) != 2:
            # numerico: no variabile da scansionare
            continue
        lhs_str = lhs_rhs[0]
        ...
        try:
            ...
            # if non parse-abile, passa oltre
            continue
    ...
    # variables effettive (p colonne). If no variabile trovata usiamo x di default.
    ...
    if symbolic_input:
        ...
        for lhs_str, rhs_str in equations:
            try:
                ...
                # Verifica che compaiano only le variables x,y,z,w e k
            ...
            # Verifica che il system sia lineare (no variabile al denominatore o grado > 1)
    ...
    A_ech = Aug_ech[:, :p]
    b_ech = Aug_ech[:, p:]
    # fattorizza eventuali polinomi in k (if non banali)
    ...
    # --- Analisi del parametro k sulla matrix semplificata -------------
    ...
    # --- Studio dei casi in function di k ---------------------------------
    ...
    for kval in cases:
        ...
        # sintesi compatibilità e tipo di solution
        ...
        # update the previous step with the correct content
        steps[-1]["content"] = content_line

        # if non compatibile, salto direttamente al prossimo caso
        if not compatible:
            continue

        # if unica, aggiungo la solution unica
        if unique:
