...
# ---------- endpoint ----------------------------------------------------- #
@cc_bp.route("/api/complementary-conditions", methods=["POST"])
...
def condizioni_complementari_route():
    ...
    # if il client non passa 'n', ricavalo dalla lunghezza di constraints
    n_raw = data.get("n")
    try:
    ...
    # update 'n' if non combacia with le constraints dichiarate
    if len(constr) != n:
        n = len(constr)

    # limiti attuali: per ora accettiamo da 1 a 6 constraints (estendibile)
    if n < 1 or n > 6:
        return jsonify(success=False,
    ...
    # ---------------------------------------------------- #
    # STEP 0 : problema inserito (constraints + segni + nonneg)
    # ---------------------------------------------------- #
    try:
    ...
    # Ricaviamo m (= n constraints) e p (= # variables originali)      
    # derive il massimo indice delle variables x_j presenti nella function obiettivo
    ...
    for sign, y in zip(signs_list, y_syms):
        if sign == "=":
            ...
            if tipo == "max":          # primale di MAX
                ...
                dual_var_sign.append((y, "<=0" if sign == "<=" else ">=0"))
    # segno delle variables duali in base sia al tipo di vincolo del primale che al tipo del problema
    dual_nonneg = []
    for sign, y in zip(signs_list, y_syms):
        if sign == "=":
    ...
    # ---------------------------------------------------- #
    # STEP 3 : feasibility (constraints + segno)
    # ---------------------------------------------------- #
    feas_checks = []
    feas = True

    if modo == "primale":
        # controllo sui constraints PRIMALI
        ...
        for (L, R, S) in constr:
            ...
            feas &= bool(ok)
        # segni variables primali (da nonneg string)
        for piece in nonneg.split(","):
            piece = piece.strip()
            if not piece:
                continue
        ...
        # controllo sui constraints DUALI
    ...
    if not feas:
        # --- conclusione per solution non ammissibile (fase 1) --------------
        steps.append({
            "title": "Conclusione:",
    ...
    # ---------------------------------------------------- #
    # STEP 4 : condizioni di complementarità → system lineare
    # ---------------------------------------------------- #
    system_eqs: List[str] = []

    if modo == "duale":
        #  Si costruisce un system in x utilizzando la regola:
        #   if y_i ≠ 0 ⇒ il vincolo i‑esimo del primale deve valere in uguaglianza
        #   if il vincolo duale j‑esimo NON è in uguaglianza ⇒ x_j = 0
        #  (gli indici i, j sono rispettivamente quelli dei constraints e delle variables)
        #
        ...
        # eventuali x_j = 0 (constraints duali *non* in uguaglianza)
        subs_duale = {y_syms[i]: sol_vec[i] for i in range(len(sol_vec))}
        ...
        # modo == primale → system nei moltiplicatori duali y
        # if x_j > 0 ⇒ vincolo j del duale in uguaglianza
        ...
        # if il vincolo primale i‑esimo NON è in uguaglianza ⇒ y_i = 0
    ...
    if sol_system is not None:
        ...
        # ---------- verifica feasibility della solution trovata ----------
        feas2 = True
        feas_checks2 = []
        if modo == "duale":
            # verifica sul primale with x‑solution
            ...
            # verifica sul duale with y‑solution
